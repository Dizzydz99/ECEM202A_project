...
Accelerometers and magnetometers have the ability to reproduce gyroscopes due to the redundant degrees of freedom in the IMU. Physical model based methods are proposed to solve the problem. In this project, I proposed an approach using machine learning and low-sample-rate gyro value to emulate virtual gyroscope. The experimental results showed that the proposed method can approximately reconstruct the gyroscope, given a reasonable step value. Also, more complicated model can be used to emulate the output data and the time-dependency of the input data can be better utilized. Another interesting problem that is worth exploring is that the computational complexity for computing the virtual gyro and the real power consumption. Also, it is worth noticing that, according to some literatures, gyro's power consumption is not significantly affected by the sample rate, which means we need to shut down and turn on the gyro frequently. Gyro-free reconstruction is another future direction to explore.
